...
export function FleetMapScreen() {
    return (
        <section className="h-screen w-full flex flex-col items-center justify-center px-8 relative overflow-hidden bg-slate-950">
            ...
            <motion.div initial={{ opacity: 0, y: 20, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.3, type: "spring", stiffness: 55 }} className="w-full max-w-3xl rounded-2xl border border-slate-700 overflow-hidden" style={{ boxShadow: "0 20px 60px -15px rgba(0,0,0,0.4)" }}>
                {/* Browser chrome */}
                ...
                <div className="relative bg-slate-800 overflow-hidden" style={{ height: 280 }}>
                    ...
                    {/* Geo-fence boundary */}
                    <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5 }} className="absolute border-2 border-dashed border-blue-400/50 rounded-2xl" style={{ left: "10%", top: "10%", width: "45%", height: "55%" }}>
                        <span className="absolute -top-2 left-2 text-[8px] text-blue-400 bg-slate-800 px-1">Airport Zone</span>
                    </motion.div>

                    {/* Vehicle pins */}
                    {PINS.map((pin, i) => (
                        <motion.div key={i} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 + i * 0.04, type: "spring", stiffness: 200 }} className={`absolute w-3 h-3 rounded-full ${pin.color} border-2 border-slate-900 shadow-sm cursor-pointer`} style={{ left: `${pin.x}%`, top: `${pin.y}%`, transform: "translate(-50%,-50%)" }} />
                    ))}

                    {/* Expanded pin */}
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="absolute bg-slate-900 border border-slate-700 rounded-xl shadow-lg p-2.5" style={{ left: "28%", top: "18%", minWidth: 140 }}>
