...
export function MaintenanceScreen() {
    ...
    return (
        <section className="h-screen w-full flex flex-col items-center justify-center px-8 relative overflow-hidden bg-slate-950">
            ...
            {/* Fleet Brain suggestion — phase 3 + extra 500ms delay */}
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={phase >= 3 ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                transition={{ delay: 0.5, duration: 0.5 }}
                className="flex items-center gap-2 bg-blue-500/10 border border-blue-500/30 rounded-xl px-4 py-2.5 mb-5 max-w-xl w-full"
            >
            ...
            <div className="grid grid-cols-3 gap-4 w-full max-w-3xl">
                {COLUMNS.map((col, ci) => (
                    <motion.div
                        key={ci}
                        initial={{ opacity: 0, y: 20 }}
                        animate={phase >= col.phase ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                        ...
                        <div className="space-y-2.5">
                            {col.cards.map((card, ki) => (
                                <motion.div
                                    ...
                                    animate={phase >= col.phase ? { opacity: 1, x: 0 } : { opacity: 0, x: -8 }}
