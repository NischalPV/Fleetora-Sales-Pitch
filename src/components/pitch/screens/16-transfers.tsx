            ...
            >
                ...
                <div className="bg-slate-900 px-5 py-4 border-b border-slate-700/50 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm font-bold text-white">TRF-20260416-007</p>
                    </div>
                    <span className="text-[10px] font-semibold px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-400">In Transit</span>
                </div>

                <div className="p-5 grid grid-cols-[1fr_auto_1fr] gap-4 items-center border-b border-slate-700/50">
                    <div className="bg-slate-900 rounded-xl p-3 border border-slate-700/50 text-center">
                        ...
                        <p className="text-lg font-bold text-white">Beach</p>
                    ...
                    <div className="bg-slate-900 rounded-xl p-3 border border-slate-700/50 text-center">
                        ...
                        <p className="text-lg font-bold text-white">Airport</p>
                        <p className="text-[10px] text-slate-400">Branch — Terminal 1</p>
                    </div>
                </div>
                        ...
                        {TIMELINE.map((step, i) => (
                            <div key={i} className="flex items-center flex-1">
                                ...
                                >
                                    ...
                                    <p className={`text-[8px] mt-1 font-semibold ${step.done ? "text-emerald-400" : step.active ? "text-amber-400" : "text-slate-600"}`}>{step.label}</p>
                                    <p className={`text-[7px] ${step.done ? "text-slate-400" : step.active ? "text-amber-500" : "text-slate-600"}`}>{step.time}</p>
                                </motion.div>
                                {i < TIMELINE.length - 1 && (
                                    <div className={`flex-1 h-0.5 mx-1 ${i < 2 ? "bg-emerald-500/50" : "bg-slate-700"}`} />
                                )}
                            </div>
                        ))}
                    </div>
