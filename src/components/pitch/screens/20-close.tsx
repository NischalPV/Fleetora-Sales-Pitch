    ...
    return (
        <section className="h-screen w-full flex flex-col items-center justify-center px-8 relative overflow-hidden bg-slate-950">
            ...
            <motion.p
                ...
                className="text-base text-slate-400 text-center mb-10 max-w-lg"
            >
                2-week pilot. One branch. Real data, real results.
            </motion.p>

            <div className="w-full max-w-xl space-y-4">
                {/* Phase 2: Timeline cards appear with stagger */}
                <div className="grid grid-cols-2 gap-4">
                    {TIMELINE_STEPS.map((step, i) => (
                        <motion.div
                            ...
                            className={`rounded-2xl border p-5 ${step.bg} ${step.border}`}
                        >
                ...
                {/* Phase 3: Training callout + contact + closing question fade in sequentially */}
                <motion.div
                    initial={{ opacity: 0, scale: 0.97 }}
                    animate={phase >= 3 ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.97 }}
                    transition={{ duration: 0.4 }}
                    className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 flex items-center gap-4"
                >
